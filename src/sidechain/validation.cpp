 ...
 * base bytes, so a 30 000 000 weight mainchain block holds about 68 800 of
 * them. Each earns one coinbase output of about 34 bytes, which is 136 weight,
 * so about 9 360 000 weight in all. Half a block clears that, and still leaves
 ...
 * more than twice what one abort may spend, so the count cap alone is not a
